...
class WC_REST_Products_Controller extends WC_REST_Legacy_Products_Controller {
	...
	public function prepare_object_for_response( $object, $request ) {
		...
		if ( $object->is_type( 'variable' ) && $object->has_child() ) {
			...
			//$data['variations_price'] = $object->get_variation_regular_price('min');
			if($object->get_variation_sale_price('min') != null){
				$data['sale_price'] = $object->get_variation_sale_price('min');
				$data['regular_price'] = $object->get_variation_regular_price('min');
			}
		...
		$schema         = array(
			...
			'properties' => array(
				...
				'variations_price'		=> array(
					'description' => __( 'Variation_price', 'woocommerce' ),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
